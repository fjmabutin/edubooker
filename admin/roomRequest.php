...
<head>
    ...
    /* MODAL */
        ...
        *{
        ...
        /* ACTIVE TAB UNDERLINE (toggled via JS using classList, not inline styles) */
        ...
        .request-list{
            flex: 1;
            min-width: 0; /* lets the column shrink below its content size so it never pushes into the details panel */
        ...
        .request-card{
            ...
            display: flex;
            flex-wrap: wrap;            /* FIX: lets card sections wrap instead of overflowing past the card / over the details panel */
        ...
        .request-user{
            ...
            min-width: 170px;           /* FIX: reduced from 230px so it can shrink when the details panel is open */
            flex: 1 1 170px;
        }
        ...
        .request-room{
            ...
            padding-left: 20px;
            min-width: 120px;           /* FIX: reduced from 180px */
            flex: 1 1 120px;
        }
        ...
        .request-purpose{
            border-left: 1px solid #ddd;
            padding-left: 20px;
            min-width: 110px;           /* FIX: reduced from 170px */
            flex: 1 1 110px;
        }
        ...
        /* FIX: added so the details panel badge can correctly reflect Approved / Rejected, not just Pending */
        .approved-status{
            background: #d9f3dc;
            color: green;
            ...
            <div class="request-card">
                ...
                <div class="request-actions">
                    ...
                    <button class="details-btn"
                        data-name="Francine Joy D. Mabutin"
                        data-id="2025-01111-MN-0"
                        data-img="../assets/FrancineJoy.png"
                        data-room="COMLAB 202"
                        data-date="March 22, 2026"
                        data-time="9:00AM - 11:00AM"
                        data-purpose="Research">
...
<div class="request-details" id="requestDetails">
    ...
    <div class="details-profile">
        ...
        <div>
            ...
            <span class="pending-status" id="detailStatus">Pending</span>
        </div>
    </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function setStatusBadge(status) {
        ...
        badge.classList.remove('pending-status', 'approved-status', 'rejected-status');
 
        if (status === 'approved') {
            badge.classList.add('approved-status');
            badge.innerText = 'Approved';
        } else if (status === 'rejected') {
            badge.classList.add('rejected-status');
            badge.innerText = 'Rejected';
        } else {
            badge.classList.add('pending-status');
            badge.innerText = 'Pending';
        }
    }
 
    function lockCard(card, status) {
        ...
        const targetList = document.querySelector(
            `[data-tab="${status === 'approved' ? 'approved' : 'rejected'}"] .request-list`
        );
    ...
    document.querySelectorAll('.details-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            setStatusBadge(selectedCard.dataset.status || 'pending');
        });
    });
 
    document.getElementById('detailApproveBtn').addEventListener('click', () => {
        if (!selectedCard) return;
        lockCard(selectedCard, 'approved');
    ...
    document.getElementById('detailRejectBtn').addEventListener('click', () => {
        if (!selectedCard) return;
        lockCard(selectedCard, 'rejected');
        detailsPanel.classList.remove('active');
        selectedCard = null;
    });
    ...
    document.querySelectorAll('.action-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btn.dataset.action === "approve") {
                ...
                confirmBtn.classList.remove('reject-btn');
                confirmBtn.classList.add('approve-btn');
                ...
                confirmBtn.classList.remove('approve-btn');
                confirmBtn.classList.add('reject-btn');
    ...
    document.getElementById('confirmBtn').addEventListener('click', function () {
        ...
        lockCard(selectedCard, action === 'approve' ? 'approved' : 'rejected');
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            let tabName = 'pending';
            if (tab.classList.contains('approved-tab')) tabName = 'approved';
            if (tab.classList.contains('rejected-tab')) tabName = 'rejected';
